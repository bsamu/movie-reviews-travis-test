...
app.post("/api/login", async (req, res) => {
  ...
  const sessionToken = await jwt.sign({ id: userId }, process.env.JWT_SECRET, {
    expiresIn: "1h",
  }); //here we're creating our own JWT, signing it with our own JWT secret, then sending it back to the frontend, where it will be saved to the sessionStorage
  res.json(sessionToken);
});

app.post("/api/reviews/add", checkSession(), async (req, res) => {
  //adding a new review
  const { token, movieId, movieTitle, content, rating } = req.body;
